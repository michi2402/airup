...
def get_all_questions(filename):
    """
        ...
        Args:
            filename (str): Path to a JSON file containing a questions with snippets

        Returns:
            tuple:
                - questions (List[str]): A list of question bodies (strings).
                - questions_with_snippets (List[List]): A list of snippet lists,
                  where each inner list contains the snippets for the corresponding question.
                  If a question has no snippets, an empty list is returned for that entry.
    """
...
def save_data_from_file(filename):
    """
        ...
        Args:
            filename (str): Path to the input JSON file. The file is expected to
                            contain a "questions" field, where each entry includes
                            a "body" (question text) and optionally "snippets".

        Returns:
            tuple:
                - test_dataset (Dataset): The test portion of the processed dataset.
                - all_questions (List[str]): A list of all question texts from the dataset.
    """
    with open(filename) as f:
        data = json.load(f)
...
def get_training_data(filename):
    ...
    Args:
        filename (str): Path to the original JSON file containing raw question and snippet data.

    Returns:
        train_dataset (Dataset): A HuggingFace Dataset object containing the training samples.
    """
    filename_for_save = filename[len(DATA_FOLDER):].split(".")[0]
...
def get_testing_data(filename):
    """
        ...
        Args:
            filename (str): Path to the original JSON file used to identify the
                            corresponding saved test dataset.

        Returns:
            grouped_data (Dict[str, List[Tuple[str, str, float]]]):
                A dictionary where each key is a question string, and each value is
                a list of tuples containing:
                    - snippet text (str)
                    - document name/path (str)
                    - relevance label (float)
    """
    filename_for_save = filename[len(DATA_FOLDER):].split(".")[0]
...
def process_data(path):
    ...
    Args:
        path (str): Path to the JSON file containing a "questions" list. Each question
                    must include a "body" and "id", and may include "snippets".

    Returns:
        question_groups (Dict[str, List[Tuple[str, str, int, str, Dict]]]):
            A dictionary where each key is a question string, and each value is a list
            of tuples, one for each snippet, containing:
                - snippet text (str)
                - document name (str)
                - relevance score (int), based on inverse snippet order
                - question ID (str)
                - original snippet object (Dict)
    """
    with open(path) as f:
        data = json.load(f)
...
def process_data_for_docs(path):
    ...
    Args:
        path (str): Path to the JSON file containing question data.

    Returns:
        question_groups (Dict[str, List[Tuple[str, Any, int, str]]]):
            A dictionary where each key is a question string, and each value is a list of tuples,
            each representing a document wish with:
                - key (str): The question.
                - value (Any): The corresponding value for that key.
                - score (int): An integer score calculated as `len(q) - i`.
                - question ID (str): The ID of the question.
    """
    with open(path) as f:
        data = json.load(f)
